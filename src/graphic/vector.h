...
struct coord {
    ...
    CUCALL coord(float x, float y, float z)
      : x{x}
      , y{y}
      , z{z}
      , w{1}
...
/// Euclid norm.
inline CUCALL float norm(const coord v) noexcept
{
...
{
    auto N = norm(v);
    return coord{v.x / N, v.y / N, v.z / N};
}
...
{
    return coord{lhs.y * rhs.z - lhs.z * rhs.y, lhs.z * rhs.x - lhs.x * rhs.z,
