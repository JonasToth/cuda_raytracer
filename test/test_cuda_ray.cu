...
TEST(ray, trace_many_failing)
{
    ...
    ASSERT_EQ(LIB::none_of(thrust::device, Result.begin(), Result.end(), does_intersect{}),
              true) << bwOutput(Result, SquareDim);

    std::cout << bwOutput(Result, SquareDim) << std::endl;
    OUT << "BW output done" << std::endl;
}

TEST(ray, trace_perspective)
{
    constexpr std::size_t SquareDim = 30;
    thrust::device_vector<coord> Vertices(3);
    Vertices[0] = {.5f,-1,1}; 
    Vertices[1] = {-1,.5f,1};
    Vertices[2] = {1,1,1};
    ...
    const auto HitCount = LIB::count_if(thrust::device, Result.begin(), Result.end(), 
                                        does_intersect{});
    ASSERT_GT(HitCount, 0.1 * SquareDim * SquareDim) << bwOutput(Result, SquareDim) +
                                                        "More hits are expected\n";
    ASSERT_LT(HitCount, 0.5 * SquareDim * SquareDim) << bwOutput(Result, SquareDim) +
                                                        "Less hits are expected\n";
