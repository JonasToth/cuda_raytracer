                                                      ...
                                                      const std::vector<ray>& AllRays)
{
    ...
    LIB::transform(LIB::begin(AllRays), LIB::end(AllRays), LIB::begin(Result),
                   [&T](const ray& Ray) { return Ray.intersects(T); });
    return Result;
}
...
TEST(ray, trace_many_successfull)
{
    ...
    const auto ContainsHit =
        LIB::any_of(LIB::begin(Result), LIB::end(Result),
    ...
    const auto HitCount =
        LIB::count_if(LIB::begin(Result), LIB::end(Result),
                      [](const LIB::pair<bool, intersect>& r) { return r.first; });
    ASSERT_GT(HitCount, 0.3 * SquareDim * SquareDim)
        << bwOutput(Result, SquareDim) + "More hits are expected\n";
    ASSERT_LT(HitCount, 0.8 * SquareDim * SquareDim)
        << bwOutput(Result, SquareDim) + "Less hits are expected\n";
