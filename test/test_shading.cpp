...
TEST(shading, diffuse_coeff)
{
    ...
    EXPECT_NE(ExpectedOk, 0.f) << "diffuse material and light should return !=0.f";
    OUT << "Good Value: " << ExpectedOk << std::endl;
...
TEST(shading, complex_shade_one_channel)
{
    ...
    for (float x_dir = 2.f; x_dir > 0.f; x_dir -= 0.5f) {
        const auto lv = phong_shading(&m, 0.1, normalize(coord(x_dir, 0.f, -1.f)), hit, &ls,
                                      1ul, &T, 1ul, flat_shading_tag{}, no_shadow_tag{});
