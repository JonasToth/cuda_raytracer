...
static void BM_SceneRender(benchmark::State& state)
{
    ...
    camera c(win.getWidth(), win.getHeight(), {0.0f, 0.5f, 2.5f}, {0.01f, 0.f, -1.f});
...
BENCHMARK(BM_SceneRender)->Unit(benchmark::kMicrosecond)->MinTime(1.0);
BENCHMARK(BM_SceneDepth)->Unit(benchmark::kMicrosecond)->MinTime(1.0);
